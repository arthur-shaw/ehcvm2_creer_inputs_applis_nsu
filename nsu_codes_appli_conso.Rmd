...
show_valid_codes <- function(
    ...
    tailles_txt <- df_produit %>%
        dplyr::distinct(produit, taille) %>%
        dplyr::mutate(taille_nom = haven::as_factor(taille, levels = "labels")) %>%
        glue::glue_data("{taille_nom}...{taille}") %>%
        glue::glue_collapse(sep = "<br>")
    ...
    tailles_groupe_txt <- df_groupe %>%
        dplyr::distinct(taille) %>%
        dplyr::mutate(taille_nom = haven::as_factor(taille, levels = "labels")) %>%
        glue::glue_data("{taille_nom}...{taille}") %>%
        glue::glue_collapse(sep = "<br>")   
